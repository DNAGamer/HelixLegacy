def helpmusic():
    helpmsg = "```"
    helpmsg += "===Music===\n"
    helpmsg += "/getout            <--Disconnects from the current voice channel\n"
    helpmsg += "/spawn             <--Joins the voice channel you are in\n"
    helpmsg += "/play [song]       <--Play a song via name or url\n"
    helpmsg += "/stream            <--Play a stream via url\n"
    helpmsg += "/volume [number]   <--Volume control\n"
    helpmsg += "/skip              <--Skips the song?\n"
    helpmsg += "/pause             <--Pauses the song\n"
    helpmsg += "/resume            <--Resumes the song\n"
    helpmsg += "/shuffle           <--Shuffles the playlist\n"
    helpmsg += "/playlist          <--Shows current playlist\n"
    helpmsg += "/np                <--Shows whats Now Playing\n"
    helpmsg += "/loop [number]     <--Loop a set song a set amount of times\n"
    helpmsg += "/remove [number]   <--Remove a selected track from the playlist\n"
    helpmsg += "/clear             <--Clear the playlsist (dont be a dick)\n"
    helpmsg += "/supported         <--All the links Helix supports\n"
    helpmsg  += "\n--Playlist Generators--\n"
    helpmsg += "/electronic        <--Creates a playlist of electronic songs\n"
    helpmsg += "/rock              <--Creates a playlist of rock songs\n"
    helpmsg += "/rap               <--Creates a playlist of rap songs\n"
    helpmsg += "/retro             <--Creates a playlist of retro songs\n"
    helpmsg += "/metal             <--Creates a playlist of metal songs\n"
    helpmsg += "/jazz              <--Creates a playlist of jazz songs\n"
    helpmsg += "/classical         <--Creates a playlist of classical songs\n"
    helpmsg += "/japanese          <--Creates a playlist of japanese songs\n"
    helpmsg += "/add               <--Create a playlist made from urls on your pastebin\n"
    helpmsg += "/dump              <--Dump the entire playlist into a file\n"
    helpmsg += "```"
    return helpmsg
...
def helputility():
    helpmsg = "```"
    helpmsg += "===Utility===\n"
    helpmsg += "/helpmsg           <--This message\n"
    helpmsg += "/id                <--Get your ID\n"
    helpmsg += "/bug               <--use this to report bugs\n"
    helpmsg += "/join              <--Add me to your server or join my server\n"
    helpmsg += "/donate            <--Donate to the project and helpmsg keep the bot alive\n"
    helpmsg += "/feature           <--Request a feature to be added to Helix\n"
    helpmsg += "/info              <--General Information about the bot\n"
    helpmsg += "/updatelog         <--Information about what was in the last update\n"
    helpmsg += "/ping              <--Ping the bot\n"
    helpmsg += "/google            <--Google Search\n"
    helpmsg += "/server            <--Server Info\n"
    helpmsg += "/whois [user]      <--Info on user\n"
    helpmsg += "/urban             <--Urban Dictionary\n"
    helpmsg += "```"
    return helpmsg
...
def helpadmin():
    helpmsg = "```"
    helpmsg += "===Server Admin Utilities===\n"
    helpmsg += "/apocalypse        <--Delete all messages in the current channel\n"
    helpmsg += "/listids           <--Get all the IDs from the server\n"
    helpmsg += "/clean             <--Remove all Helix related spam\n"
    helpmsg += "/kick              <--Kick a mentioned user\n"
    helpmsg += "/ban               <--Ban a mentioned user\n"
    helpmsg += "/leaveserver       <--Make the bot leave the server\n"
    helpmsg += "/rank disable      <--Disable ranking\n"
    helpmsg += "/rank enable       <--Re-enable ranking\n"
    helpmsg += "/leaderboard       <--Your servers chat leaderboard\n"
    helpmsg += "/prefix            <--Change the prefix\n"
    helpmsg += "```"
    return helpmsg
...
def helpchat():
    helpmsg = "```"
    helpmsg += "===chat===\n"
    helpmsg += "/savage            <--Insults\n"
    helpmsg += "/compliment       <--Compliments\n"
    helpmsg += "/vicky             <--Victorian insults\n"
    helpmsg += "/8ball[question]   <--Magic 8-ball\n"
    helpmsg += "/shitpost          <--A random shitpost message\n"
    helpmsg += "/flip              <--Flip coins...and something else\n"
    helpmsg += "/doge [words]      <--generate a doge meme\n"
    helpmsg += "/echo [text]       <--echo what you said\n"
    helpmsg += "/rank              <--What is your rank?\n"
    helpmsg += "/fakekick          <--Troll kick\n"
    helpmsg += "/fakeban           <--Troll ban\n"
    helpmsg += "/cat               <--I want to see cats, and i want them now\n"
    helpmsg += "/dog               <--Give me my doggo\n"
    helpmsg += "```"
    return helpmsg
...
def helpadult():
    helpmsg = "```"
    helpmsg += "===adult===\n"
    helpmsg += "/pone helpmsg         <--Pony porn search\n"
    helpmsg += "/rule34 [search]   <--Rule34 search\n"
    helpmsg += "```"
    return helpmsg
...
( ? L? ?? ) Hello Kripparian. Reginald and Dan Dinh, Owners of Team SoloMid™ here. We gained access to your Hearthstone account and couldn't helpmsg but notice the absence of the "legendary" card back. This poorly reflects on Team SoloMid, and, as such, we have decided to remove you from the team. Please return your jacket. Thank you ( ? L? ?? )
